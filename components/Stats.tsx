...
import { BarChart3, Github, Code2, ExternalLink, Loader2, Calendar, TrendingUp } from 'lucide-react';
...
const RetroPieChart: React.FC<PieChartProps> = ({ easy, medium, hard, total }) => {
  ...
  const createPiePath = (startAngle: number, endAngle: number, offset: number = 0) => {
    const centerX = 90;
    const centerY = 90;
    const radius = 70;
    ...
    <div className="relative w-48 h-48">
      ...
      >
        ...
        <g className="transition-all duration-300 ease-out">
          <path
            d={createPiePath(-90, -90 + easyAngle, hoveredSegment === 'easy' ? 8 : 0)}
            fill="#00ffff"
            stroke="#00ffff"
            strokeWidth="2"
            style={{
              filter: hoveredSegment === 'easy' ? 'drop-shadow(0 0 8px rgba(0,255,255,0.8))' : 'drop-shadow(0 0 2px rgba(0,255,255,0.4))',
        ...
        <g className="transition-all duration-300 ease-out">
          <path
            d={createPiePath(-90 + easyAngle, -90 + easyAngle + mediumAngle, hoveredSegment === 'medium' ? 8 : 0)}
            fill="#ffd700"
            stroke="#ffd700"
            strokeWidth="2"
            style={{
              filter: hoveredSegment === 'medium' ? 'drop-shadow(0 0 8px rgba(255,215,0,0.8))' : 'drop-shadow(0 0 2px rgba(255,215,0,0.4))',
        ...
        <g className="transition-all duration-300 ease-out">
          <path
            d={createPiePath(-90 + easyAngle + mediumAngle, -90 + easyAngle + mediumAngle + hardAngle, hoveredSegment === 'hard' ? 8 : 0)}
            fill="#ff6b9d"
            stroke="#ff6b9d"
            strokeWidth="2"
            style={{
              filter: hoveredSegment === 'hard' ? 'drop-shadow(0 0 8px rgba(255,107,157,0.8))' : 'drop-shadow(0 0 2px rgba(255,107,157,0.4))',
          ...
          <div className="bg-slate-900/95 border-2 px-3 py-2 rounded-sm shadow-xl animate-fadeIn">
            {hoveredSegment === 'easy' && (
              <div className="text-center">
                <span className="font-pixel text-xs text-cyan-300 block">EASY</span>
                <span className="font-pixel text-2xl text-cyan-400">{easy}</span>
            ...
            {hoveredSegment === 'medium' && (
              <div className="text-center">
                <span className="font-pixel text-xs text-yellow-300 block">MEDIUM</span>
                <span className="font-pixel text-2xl text-yellow-400">{medium}</span>
                <span className="font-pixel text-xs text-gray-400 block">
            ...
            {hoveredSegment === 'hard' && (
              <div className="text-center">
                <span className="font-pixel text-xs text-pink-300 block">HARD</span>
                <span className="font-pixel text-2xl text-pink-400">{hard}</span>
                <span className="font-pixel text-xs text-gray-400 block">
  ...
  useEffect(() => {
    const fetchLeetCodeStats = async () => {
      try {
        ...
        // Fallback to default values silently
        setStats({
          totalSolved: 300,
          easySolved: 120,
          mediumSolved: 150,
            ...
            <div className="bg-slate-800 border-2 border-dashed border-slate-700 p-6 rounded-sm w-full h-full flex flex-col justify-center">
              ...
              <div className="flex items-center justify-between mt-6 pt-3 border-t border-slate-700">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-pixel text-[9px] text-gray-500">MORE</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-slate-900 border-4 border-retro-yellow p-6 relative flex flex-col h-full">
          ...
          {loading ? (
            <div className="mt-4 flex flex-col items-center justify-center h-64">
              ...
              <p className="font-retro text-lg text-gray-400 mt-4">Loading stats...</p>
            ...
            <div className="mt-4 flex flex-col items-center">
              ...
              <div className="w-full text-right pt-4 border-t border-slate-700 mt-4">
                <span className="font-retro text-lg text-gray-400">
                  GeeksForGeeks: <span className="text-retro-green">{gfgStats.total}+</span> Solved
                </span>
              </div>
            </div>
          )}

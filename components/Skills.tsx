...
import { Cpu, Code, Database, Globe, Wrench, Zap, ChevronRight, Star } from "lucide-react";
...
const PixelBar = ({ level, colorClass }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <div className={`absolute -top-2 -left-2 w-6 h-6 border-t-4 border-l-4 ${categories[activeCategory].borderColor}`} />
        <div className={`absolute -top-2 -right-2 w-6 h-6 border-t-4 border-r-4 ${categories[activeCategory].borderColor}`} />
        <div className={`absolute -bottom-2 -left-2 w-6 h-6 border-b-4 border-l-4 ${categories[activeCategory].borderColor}`} />
        <div className={`absolute -bottom-2 -right-2 w-6 h-6 border-b-4 border-r-4 ${categories[activeCategory].borderColor}`} />
        ...
        <div className="bg-slate-900/80 border-2 border-slate-700 p-6 md:p-8">
          ...
          <div className="flex items-center gap-3 mb-8 pb-4 border-b border-slate-700">
            <div className={`p-2 ${categories[activeCategory].bgColor}/20 border ${categories[activeCategory].borderColor}`}>
              {categories[activeCategory].icon}
            </div>
            <div>
              <h3 className={`font-pixel text-lg ${categories[activeCategory].textColor}`}>
            ...
            <div className="ml-auto flex items-center gap-1">
              ...
              <span className={`font-pixel text-xs ${categories[activeCategory].textColor}`}>
                LVL {Math.round(categories[activeCategory].items.reduce((a, b) => a + b.level, 0) / categories[activeCategory].items.length)}
              </span>
            </div>
          </div>
          ...
          <div className="mt-8 pt-4 border-t border-slate-700 flex flex-wrap justify-center gap-6">
            <div className="text-center">
              <p className={`font-pixel text-2xl ${categories[activeCategory].textColor}`}>
                {categories[activeCategory].items.filter((s) => s.level >= 90).length}
              </p>
              <p className="font-pixel text-[10px] text-gray-500 mt-1">MASTERED</p>
            </div>
            <div className="w-px bg-slate-700" />
            <div className="text-center">
              <p className={`font-pixel text-2xl ${categories[activeCategory].textColor}`}>
                {categories[activeCategory].items.filter((s) => s.level >= 70).length}
              </p>
              <p className="font-pixel text-[10px] text-gray-500 mt-1">PROFICIENT</p>
            </div>
            <div className="w-px bg-slate-700" />
            <div className="text-center">
              <p className={`font-pixel text-2xl ${categories[activeCategory].textColor}`}>
                {Math.round(
                  categories[activeCategory].items.reduce((a, b) => a + b.level, 0) /
                    categories[activeCategory].items.length
                )}
                %
              </p>
              <p className="font-pixel text-[10px] text-gray-500 mt-1">AVG POWER</p>
            </div>
          </div>
        </div>
      </div>

...
const RecentWork: React.FC = () => {
  ...
  return (
    <section id="recent-work" className="py-16 px-4 md:px-8 max-w-6xl mx-auto border-t-4 border-dashed border-gray-700">
        ...
        >
            ...
            <div className="bg-[#2c2c2c] p-6 rounded-[2rem] border-b-8 border-r-8 border-[#1a1a1a] shadow-2xl relative w-full">
              ...
              <div className="bg-[#0f0f0f] p-2 rounded-xl shadow-inner border-4 border-[#1f1f1f]">
                 ...
                 <div className="bg-black overflow-hidden relative aspect-video rounded-lg shadow-[inset_0_0_20px_rgba(0,0,0,1)] border border-gray-800 group">
                    ...
                    <div className="absolute inset-0 z-10 pointer-events-none">
                        ...
                        <div className="absolute top-0 right-0 w-full h-16 bg-gradient-to-b from-white/5 to-transparent skew-x-12"></div>
                    </div>
                    ...
                    <div className="absolute bottom-0 left-0 w-full bg-black/80 border-t border-retro-green p-4 transform translate-y-full group-hover:translate-y-0 transition-transform duration-300 z-20 flex justify-between items-center backdrop-blur-sm">
                        <div>
                            <h3 className="font-pixel text-white text-sm sm:text-lg">{currentWork.title}</h3>
                            <p className="font-retro text-retro-green text-sm">{currentWork.desc}</p>
                        </div>
                        <a href={currentWork.url} target="_blank" rel="noopener noreferrer">
                             <RetroButton variant="primary" className="text-[10px] py-2">
                                <span className="flex items-center gap-1">VISIT <ExternalLink size={12}/></span>
                             </RetroButton>
                        </a>
                    </div>
                 </div>
              ...
              <div className="text-center mt-1">
                 <span className="font-pixel text-[10px] text-gray-500 tracking-[0.2em] uppercase">Sony Trinitron</span>
              </div>
            </div>

            {/* Mobile Controls & Indicator */}
            <div className="flex justify-between items-center w-full mt-4 md:hidden px-4">
                 <button onClick={prevSlide} className="bg-slate-800 p-2 border border-white text-white"><ChevronLeft/></button>
                 <span className="font-pixel text-xs text-white">{currentIndex + 1} / {works.length}</span>
                 <button onClick={nextSlide} className="bg-slate-800 p-2 border border-white text-white"><ChevronRight/></button>
            </div>

            {/* Tech Stack Tags (Below Monitor) */}
            <div className={`mt-6 flex flex-wrap justify-center gap-2 transition-all duration-300`}>
                 {currentWork.tech.map(t => (
                    <span key={t} className={`font-pixel text-xs px-3 py-1 border-2 bg-slate-900 text-white ${currentWork.color.replace('border', 'border')}`}>
                        {t}
                    </span>
                 ))}
